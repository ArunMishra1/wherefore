...
from wherefore.comparison.key_matching import content_sanity_check, fuzzy_match_keys
...
def _apply_fuzzy_key_resolution(source_df, target_df, join_column):
    ...
    After fuzzy_match_keys proposes matches from key strings alone,
    content_sanity_check re-checks each proposed match against the
    rows' actual content before it's applied -- confirmed by direct
    testing (PERFORMANCE.md Round 8) that key-string similarity alone
    can confidently merge two genuinely unrelated records whose keys
    happen to differ only in separator/case. Matches rejected by the
    content check are treated exactly like fuzzy_match_keys' own
    unmatched_target_keys: left unrenamed, surfacing honestly as
    source-only/target-only rows rather than a silently-wrong merge.
    """
    ...
    accepted_pairs, rejected_keys = content_sanity_check(
        source_df, target_df, join_column, match_result.matched_pairs
    )

    if rejected_keys:
        preview = rejected_keys[:5]
        suffix = " ..." if len(rejected_keys) > 5 else ""
        typer.secho(
            f"Warning: {len(rejected_keys)} key(s) had a fuzzy key match but failed "
            f"the content sanity check (the matched rows' other columns don't look "
            f"like the same record) and were not auto-resolved: {preview}{suffix}",
            fg=typer.colors.YELLOW,
            err=True,
        )
        for rk in rejected_keys:
            match_result.confidence_by_target_key.pop(rk, None)

    target_df = target_df.copy()
    target_df[join_column] = target_df[join_column].astype(str).map(
        lambda k: accepted_pairs.get(k, k)
    )
