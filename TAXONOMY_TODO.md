...
- [Taxonomy build log](#taxonomy-build-log)
  - [Contents](#contents)
  - [Reasoning layer (reasoning/explain.py, reasoning/providers/)](#reasoning-layer-reasoningexplainpy-reasoningproviders)
  - [Eval harness (evals/fixtures/, evals/harness/)](#eval-harness-evalsfixtures-evalsharness)
  - [Future, deliberately deferred (not now)](#future-deliberately-deferred-not-now)
  - [Why patterns are built one at a time, not all-YAML-first](#why-patterns-are-built-one-at-a-time-not-all-yaml-first)
  - [Remaining patterns (build in this order, easiest signature first)](#remaining-patterns-build-in-this-order-easiest-signature-first)
  - [Order rationale](#order-rationale)
  - [null\_type\_coercion: three real bugs found building one pattern](#null_type_coercion-three-real-bugs-found-building-one-pattern)
  - [float\_precision: a signature-design lesson](#float_precision-a-signature-design-lesson)
  - [First real --llm eval run (against the actual Anthropic API)](#first-real---llm-eval-run-against-the-actual-anthropic-api)
  - [Multi-source format support: Parquet and Excel](#multi-source-format-support-parquet-and-excel)
  - [Multi-source roadmap: what's next](#multi-source-roadmap-whats-next)
  - [Redaction layer: data safety before any new connector](#redaction-layer-data-safety-before-any-new-connector)
  - [Batch mode: compare-dir](#batch-mode-compare-dir)
  - [S3 support](#s3-support)
  - [encoding\_mismatch: the 6th pattern](#encoding_mismatch-the-6th-pattern)
  - [Clustering extension for row-presence patterns (key\_mismatch, dedup\_failure)](#clustering-extension-for-row-presence-patterns-key_mismatch-dedup_failure)
  - [dedup\_failure: built, using the row-presence extension above](#dedup_failure-built-using-the-row-presence-extension-above)
  - [key\_mismatch: built, with a real architecture fork and a real false positive caught mid-build](#key_mismatch-built-with-a-real-architecture-fork-and-a-real-false-positive-caught-mid-build)
  - [PyPI packaging: a real, severe bug caught before it could ship](#pypi-packaging-a-real-severe-bug-caught-before-it-could-ship)
  - [Database connectivity: SQLite built, Postgres/MySQL deliberately deferred](#database-connectivity-sqlite-built-postgresmysql-deliberately-deferred)
  - [PostgreSQL: built and verified against a REAL Postgres server, not mocked](#postgresql-built-and-verified-against-a-real-postgres-server-not-mocked)
  - [Documentation: ARCHITECTURE.md and troubleshooting.md added](#documentation-architecturemd-and-troubleshootingmd-added)
  - [compare-dir database batch mode: db://\* vs db://\*](#compare-dir-database-batch-mode-db-vs-db)
